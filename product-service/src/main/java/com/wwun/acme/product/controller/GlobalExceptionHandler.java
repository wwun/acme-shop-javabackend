...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<HandlerExceptionDTO> handleProductNotFoundException(ProductNotFoundException e){
        HandlerExceptionDTO error = setErrorValues("PRODUCT_NOT_FOUND", e.getMessage(), HttpStatus.NOT_FOUND.value(), Instant.now());
    ...
    public ResponseEntity<HandlerExceptionDTO> handleIllegalArgumentException(IllegalArgumentException e) {
        HandlerExceptionDTO error = setErrorValues("INVALID_ARGUMENT", e.getMessage(), HttpStatus.BAD_REQUEST.value(), Instant.now());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
    }

    @ExceptionHandler(CategoryNotFoundException.class)
    public ResponseEntity<HandlerExceptionDTO> handleCategoryNotFoundException(CategoryNotFoundException e){
        HandlerExceptionDTO error = setErrorValues("INVALID_CATEGORY", e.getMessage(), HttpStatus.NOT_FOUND.value(), Instant.now());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResponseEntity<HandlerExceptionDTO> handleValidationErrors(MethodArgumentNotValidException e){
        HandlerExceptionDTO error = setErrorValues("VALIDATION_ERROR", e.getMessage(), HttpStatus.BAD_REQUEST.value(), Instant.now());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
    }

    @ExceptionHandler(HttpMessageNotReadableException.class)
    public ResponseEntity<HandlerExceptionDTO> handleInvalidFormat(HttpMessageNotReadableException e){
        HandlerExceptionDTO error = setErrorValues("MALFORMED_JSON",e.getMessage(), HttpStatus.BAD_REQUEST.value(), Instant.now());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
    }
    ...
    public HandlerExceptionDTO setErrorValues(String errorType, String message, int statusCode, Instant timestamp){
        return new HandlerExceptionDTO(errorType, message, statusCode, timestamp);        
    }
